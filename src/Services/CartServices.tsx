...
export default class CartServices {
    private static readonly API_KEY: string = (process.env.REACT_APP_COMMERCE_TEST_KEY ? process.env.REACT_APP_COMMERCE_TEST_KEY : '');
    private static readonly BASE_URL: string = 'https://api.chec.io/v1/';

     public async createCart(key: string | null): Promise<cartResource> {
        let url = '';
        if (!key) {
    ...
    public async addItemToCart(cartId: string, itemId: string ): Promise<cartResource> {
        const url = CartServices.BASE_URL + `carts/${cartId}`;
        console.log(url);
        //variant_id can be added here
        const body = {id: itemId, quantity: 1}
    ...
    public async updateItemInCart(cartId: string, lineItemId: string, quantity: number ): Promise<cartResource> {
    ...
    public async emptyItemFromCart(cartId: string, lineItemId: string): Promise<cartResource> {
        const url = CartServices.BASE_URL + `carts/${cartId}/items/${lineItemId}`;
